...
```
Install gcovr in your computer (running Linux), run this command on terminal: `sudo apt-get install gcovr`
...
Ex: type `1` then hit `Enter` to see result in console only.
...
To install doxygen in your computer (running Linux), run this command on terminal: `sudo apt-get install doxygen`
Then run below command to generate html document:
```
$ cd build
$ doxygen Doxifile.out
```
The output file will be in `build/docs/html`
Open `index.html` on a web browser to see document generated by Doxygen
